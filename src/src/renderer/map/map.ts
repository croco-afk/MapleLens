...
export class MapleMap extends Container {
  ...
  toggleLayerVisibility(disabledLayers: string[]) {
    ...
    for (const layer of disabledLayers) {
      switch (layer) {
        case 'background': {
          const bottomLayer = this.layers[this.bottomLayer];
          if (bottomLayer !== undefined) {
            bottomLayer.visible = false;
          }
          break;
        }
        case 'foreground': {
          const topLayer = this.layers[this.topLayer];
        ...
        case 'particle': {
          const particleLayer = this.layers[this.particleLayer];
          if (particleLayer !== undefined) {
